  ...
  <div class="orbit-container">
    <div class="orbit-ring">
      <div
        ...
        class="orbit-icon"
        :style="{
          '--angle': `${(360 / icons.length) * index}deg`,
          '--delay': `${index * 0.1}s`,
        }"
      >
        {{ icon }}
    ...
    <div class="avatar-wrapper">
      <img :src="currentAvatarImage" alt="Caio Kinzel Filho" class="avatar" />
    </div>
  </div>
</template>
...
<script setup lang="ts">
  ...
  const icons = ["vue", "ts", "go", "node", "k8s", "aws"];
...
<style scoped>
  ...
  @keyframes fadeIn {
    from {
      opacity: 0;
      transform: rotate(var(--angle)) translateX(100px) rotate(calc(var(--angle) * -1)) scale(0.8);
    }
    to {
      opacity: 1;
      transform: rotate(var(--angle)) translateX(130px) rotate(calc(var(--angle) * -1)) scale(1);
    }
  }
  ...
  @media (--md) {
    ...
    @keyframes fadeIn {
      from {
        opacity: 0;
        transform: rotate(var(--angle)) translateX(70px) rotate(calc(var(--angle) * -1)) scale(0.8);
      }
      to {
        opacity: 1;
        transform: rotate(var(--angle)) translateX(100px) rotate(calc(var(--angle) * -1)) scale(1);
      }
    }
