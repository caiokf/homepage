...
export class GoogleSheetsProvider implements TechRadarDataProvider {
  ...
  async fetchVersion(versionId: string): Promise<TechRadarData> {
    ...
    const url = `${GOOGLE_SHEETS_API_BASE}/${this.config.sheetId}/values/${encodeURIComponent(range)}?key=${this.config.apiKey}`;
  ...
  private async ensureMetadata(): Promise<void> {
    ...
    const url = `${GOOGLE_SHEETS_API_BASE}/${this.config.sheetId}?key=${this.config.apiKey}`;
